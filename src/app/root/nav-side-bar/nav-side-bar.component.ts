import { clinicManagementConst, urlPaths } from './../../config/constants/clinicManagementConstants';
...
import { Roles, Token_Role } from '../../config/enums/clinicManagement.enum';
...
export class NavSideBarComponent implements OnInit {
    ...
    title: string;
    sidebar = [];
    Username: any = clinicManagementConst.username;
    menuItems = clinicManagementConst.menu;
    selectedRow: number;
    ...
    initiateVariables() {
        this.title = clinicManagementConst.siteName.name;
    }
    ...
    getUsername() {
        this.rootService.getUser().subscribe((res) => {
            if (res) {
                ...
                this.Username = clinicManagementConst.username.name;
            }
        });
    }
    setRole() {
        this.rootService.getTokenRole().pipe(first()).subscribe((res) => {
            ...
            if (res) {
                ...
                this.userRole = Token_Role.ROLE_DOCTOR;
            }
            this.makeSideBar();
        });
    }
    makeSideBar() {
        ...
        if (this.userRole) {
            auth = true;
            _.forEach(clinicManagementConst.sideBar, (res) => {
